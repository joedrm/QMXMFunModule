...
- (void)startPlayBackWithStartDate:(NSDate *)date endDate: (NSDate *)endDate{
    struct H264_DVR_FINDINFO requestInfo;
    memset(&requestInfo, 0, sizeof(H264_DVR_FINDINFO));
    requestInfo.nChannelN0 = self.channel;
    requestInfo.nFileType = 0;
    ...
    requestInfo.startTime.dwMonth = [NSDate getMonthFormDate:date];
    requestInfo.startTime.dwDay = [NSDate getDayFormDate:date];
    requestInfo.startTime.dwHour = [NSDate getHourFormDate:date];
    requestInfo.startTime.dwMinute = [NSDate getMinuteFormDate:date];
    requestInfo.startTime.dwSecond = [NSDate getSecondFormDate:date];
    
    requestInfo.endTime.dwYear = [NSDate getYearFormDate:endDate];
    requestInfo.endTime.dwMonth = [NSDate getMonthFormDate:endDate];
    requestInfo.endTime.dwDay = [NSDate getDayFormDate:endDate];
    requestInfo.endTime.dwHour = [NSDate getHourFormDate:endDate];
    requestInfo.endTime.dwMinute = [NSDate getMinuteFormDate:endDate];
    requestInfo.endTime.dwSecond = [NSDate getSecondFormDate:endDate];
    [self start:requestInfo];
}



- (void)playVideoWithRecord:(RecordInfo *)record {
    struct H264_DVR_FINDINFO requestInfo;
    memset(&requestInfo, 0, sizeof(H264_DVR_FINDINFO));
    requestInfo.nChannelN0 = self.channel;
    requestInfo.nFileType = 0;
    requestInfo.startTime.dwYear = record.timeBegin.year;
    requestInfo.startTime.dwMonth = record.timeBegin.month;
    requestInfo.startTime.dwDay = record.timeBegin.day;
    requestInfo.startTime.dwHour = record.timeBegin.hour;
    requestInfo.startTime.dwMinute = record.timeBegin.minute;
    requestInfo.startTime.dwSecond = record.timeBegin.second;

    requestInfo.endTime.dwYear = record.timeEnd.year;
    requestInfo.endTime.dwMonth = record.timeEnd.month;
    requestInfo.endTime.dwDay = record.timeEnd.day;
    requestInfo.endTime.dwHour = record.timeEnd.hour;
    requestInfo.endTime.dwMinute = record.timeEnd.minute;
    requestInfo.endTime.dwSecond = record.timeEnd.second;
    [self start:requestInfo];
}
...
-(void)seekToTime:(uint64)addtime
{
//    FUN_MediaSeekToTime(self.player, (int)addtime, 0, 0);
    
    FUN_MediaSeekToMSTime(self.player, addtime, 0);
}
