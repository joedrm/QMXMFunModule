...
- (void)loginWithTypeAP:(NSString *)username pwd:(NSString *)pwd mac:(NSString *)mac {
    ...
    STRNCPY(devInfo.loginPsw, SZSTR(@"sengled123"));
    // 判断直连设备的类型
    ...
    if ([sid hasPrefix:@"socket"]) {
        ...
        devInfo.nType = 0;
        devInfo.nPort = 34567;
//        strcpy(devInfo.Devmac, "ec1e9cbd0f8d18dc");
        strcpy(devInfo.Devmac, [mac UTF8String]);
    }
    FUN_DevSetLocalPwd(SZSTR(mac),SZSTR(username), SZSTR(pwd));
    FUN_SysAdd_Device(self.msgHandle, &devInfo);
    
    FUN_SysGetDevList(self.msgHandle, "", SZSTR(@"sengled123"),0);
