...
typedef struct FaceImage_Struct{
    uint picturelen;
    uint featurelen;
    uint jsonlen;
    uint resv[5];
...
- (void)getMonthPictureDate:(NSDate *)date {
    ...
    //调试
//    ChannelObject *channel1 = [[DeviceControl getInstance] getSelectChannel];
//    //图片转Data
//    UIImage *image = [UIImage imageNamed:@"radar_search_bg.png"];
//    NSData *imageData = UIImagePNGRepresentation(image);
//
//    //json转Data
//    NSNumber *numb = [NSNumber numberWithInt:100];
//    NSMutableDictionary *dic =[@{@"Name":@"FaceImport",@"FaceImport":@"0x00000002",@"FaceImport":@{@"FaceID":numb,@"Action":@"del"}} mutableCopy];
//    NSError *error;
//    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:dic options:NSJSONWritingPrettyPrinted error:&error];
//
//    //结构体转Data
//    FaceImage_Struct faceStruct = {0};
//    faceStruct.picturelen = (uint)imageData.length;
//    faceStruct.jsonlen = (uint)jsonData.length;
//    NSMutableData *stuctData = [[NSData dataWithBytes:&faceStruct length:sizeof(FaceImage_Struct)] mutableCopy];
//
//    NSLog(@"%s",(char*)[stuctData bytes]);
//
//    [stuctData appendData:imageData];
//    [stuctData appendData:jsonData];
//
////    FUN_DevCmdGeneral(SELF, [channel1.deviceMac UTF8String], 3014, "FaceImport", -1, 20000, stuctData, (int)[stuctData length] + 1, -1, 0);
//
//    return;
    //结束
