//
//  RootTransitionGestureProcess.swift
//  QNN
//
...
open class RootTransitionGestureProcess:NSObject,GestureRecognizerDelegate{
    
    open var popGestureRecognizer: UIPanGestureRecognizer
    
    open weak var container: UIViewController!{
        willSet{
            if container != .none {
    ...
    open func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        if container.qnn_popEdge == [] {
            return false
        }
        
        guard let navigationController = container.qnn_navigationController , navigationController.viewControllers.count > 1 else {
            return false
        }
        
        if container.qnn_animationProcessing.convertTransitionAnimation().interactiveTransition != nil {
            return false
        }
        
        let allowedEdge = container.qnn_popAllowedEdge
        if allowedEdge != .zero {
            let beginLocation = gestureRecognizer.location(in: container.view)
            if container.view.bounds.inset(by: allowedEdge).contains(beginLocation) {
                 return false
            }
        }
        
        if let gan = gestureRecognizer as? UIPanGestureRecognizer,!container.qnn_popEdge.contains(gan.wz_direction) {
            return false
        }
        return true
    }
    ...
    open func gestureRecognizer(_ gestureRecognizer: UIGestureRecognizer, shouldRecognizeSimultaneouslyWith otherGestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        if gestureRecognizer.state == .began && gestureRecognizer.view != otherGestureRecognizer.view {
            if let scrollView = otherGestureRecognizer.view as? UIScrollView{
                ...
                if !container.qnn_popEdge.contains(scrollDirection) {
                    return false
                }
                
                var isSlidingToEdge = false
                assert(container.qnn_popEdge.isOnly || (!container.qnn_popEdge.isOnly && allowedEdge == .zero), "多方位Pop和限定响应区域设置请自定义手势处理，此处只提供其他条件下基本返回功能")
                if container.qnn_popEdge.contains(scrollDirection){
    ...
    open func handlePopRecognizer(_ recognizer:UIPanGestureRecognizer){
        ...
        let animation = container.qnn_animationProcessing.convertTransitionAnimation()
...
open class WZRootTransitionScreenEdgeGestureProcess:NSObject,GestureRecognizerDelegate{
    ...
    open weak var container: UIViewController!{
        willSet{
            ...
            (popGestureRecognizer as! UIScreenEdgePanGestureRecognizer).edges = newValue.qnn_popEdge
            newValue.view.addGestureRecognizer(popGestureRecognizer)
        }
    }
