...
public extension UILabel {
    //同一段文案 一个部分色值、字体变化
    func attributeText(text: String, color: UIColor? = nil,font: UIFont? = nil) {
    ...
    // 同一段文案 多个部分色值、字体变化
    func attributeTexts(text: String ..., color: UIColor? = nil,font: UIFont? = nil) {
        if let labelText = self.text {
            let attr = NSMutableAttributedString(string: labelText)
    ...
    // 同一段文案 多个部分色值、字体变化
    func attributeTextsArr(textsArr: [String], color: UIColor? = nil,font: UIFont? = nil) {
        if let labelText = self.text {
            let attr = NSMutableAttributedString(string: labelText)
            // 中间变量 可变 text, 解决当传入的text有相同元素的时候，定位Range
            var mutiText = labelText
    ...
    func newAttrbuteTexts(text: String ..., color: UIColor? = nil,font: UIFont? = nil) {
